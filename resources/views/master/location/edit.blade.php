    ...
    <section class="locations-section py-4">
        <div class="container container-custom">
            <div class="content-card card card-body mb-3">
                <form action="{{ route('master.location.update', $location->location_id) }}" method="post" name="location_form">
                    @csrf
                    @method('PUT')

                    <div class="page-header pb-0">
                        <h4 class="fs22">Edit Location</h4>
                    </div>
                    <hr>
                    <div class="mb-3">
                        ...
                        <input type="text" name="name" class="form-control" id="locationName"
                            placeholder="Enter location name" value="{{ old('name') ?? $location->location->name }}">
                        @error('name')
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="description" class="form-control" id="locationDescription" rows="3"
                            placeholder="Describe the location">{{ old('description') ?? $location->location->description }}</textarea>
                        @error('description')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="username" class="form-control" id="username"
                            placeholder="Enter username" value="{{ old('username') ?? $location->user->name }}">
                        @error('username')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="phone" class="form-control" id="phone" placeholder="Enter Mobile No."
                            ...
                            value="{{ old('phone') ?? $location->user->phone }}">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="email" name="email" class="form-control" id="email" placeholder="Enter email"
                            value="{{ old('email') ?? $location->user->email }}">
                        @error('email')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
                    ...
                    <div class="mb-3 form-check">
                        <input type="checkbox" class="form-check-input" id="userActive" name="active"
                            {{ old('active', $location->user->active) ? 'checked' : '' }}>
                        <label class="form-check-label" for="locationActive">Active</label>
                    </div>
                    <div class="text-end"><button type="submit" class="btn btn-prime btn-lg px-5">Edit LOCATION</button></div>
                </form>
            </div>
        </div>
    </section>
