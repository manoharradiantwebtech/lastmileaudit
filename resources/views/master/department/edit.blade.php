...
<section class="departments-section py-4">
    <div class="container container-custom">
        <div class="content-card card card-body mb-3">
            <form action="{{ route('master.department.update', $department->id) }}" method="post" name="department_form">
                @csrf
                @method('PUT')
                <div class="page-header pb-0">
                    <h4 class="fs22">Edit Department</h4>
                </div>
                <hr>
                <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" id="departmentManager" name="user_id">
                        ...
                        @foreach ($users as $user)
                            <option value="{{ $user->id }}" {{ $user->id == $department->user_id ? 'selected' : '' }}>
                                {{ $user->name }}
                            </option>
                        @endforeach
